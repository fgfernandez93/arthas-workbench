...
- 多个 JVM 会话并存时，Console、Log、Web UI 与端口信息容易分散
...
4. 在 IDE 内提供接近原生终端体验的 Arthas Console
...
插件必须提供独立的会话窗口，并在其中管理 Console 与 Log。
...
- `Agent MCP 密码` 不是 IDEA Console 的连接密码
