...
/**
 * 单个 Arthas 会话对应的内容面板。
 * 每个页签内部只在 Console 和 Log 两种视图之间切换，不再拆成多个 Tool Window。
 */
public final class ArthasSessionTabPanel extends JPanel implements Disposable {

    private static final String CONSOLE_CARD = "console";
    ...
    private final ArthasConsolePanel consolePanel;
    private final JBTextArea logArea = new JBTextArea();
    private final JToggleButton consoleToggle =
            new JToggleButton(ArthasWorkbenchBundle.message("enum.session.view.console"));
    ...
    public ArthasSessionTabPanel(Project project, String sessionId) {
        ...
        this.consolePanel = new ArthasConsolePanel(project);
        ...
        cardPanel.add(consolePanel, CONSOLE_CARD);
    ...
    public void bindSnapshot(ArthasSessionService.SessionSnapshot snapshot) {
        consolePanel.bindSession(snapshot.getSession());
        applyLogStyle();
    ...
    public void selectView(ArthasSessionViewType viewType, boolean publishSelection) {
        ...
        syncingView = true;
        try {
            consoleToggle.setSelected(targetView == ArthasSessionViewType.CONSOLE);
            logToggle.setSelected(targetView == ArthasSessionViewType.LOG);
            cardLayout.show(cardPanel, targetView == ArthasSessionViewType.CONSOLE ? CONSOLE_CARD : LOG_CARD);
        } finally {
            syncingView = false;
        }
        if (publishSelection) {
    ...
    private JPanel createSwitchBar() {
        ...
        group.add(consoleToggle);
        group.add(logToggle);
        consoleToggle.addActionListener(event -> handleToggleSelection(ArthasSessionViewType.CONSOLE));
        logToggle.addActionListener(event -> handleToggleSelection(ArthasSessionViewType.LOG));
        panel.add(consoleToggle);
        panel.add(logToggle);
        return panel;
    }
